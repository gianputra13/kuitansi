              ...
              <tbody>
                @foreach ($data as $item)
                  <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }}">
                    <td class="border-b border-gray-300 px-4 py-2 text-blue-500 flex flex-col group">
                      {{ $item->code }}
                    ...
                    <td class="border-b border-gray-300 px-4 py-2">{{ $item->received_from }}</td>
                    <td class="border-b border-gray-300 px-4 py-2">Rp {{ number_format($item->payment_number, 0, ',' , '.' )  }}</td>
                  </tr>
                @endforeach
              </tbody>
          </table>
